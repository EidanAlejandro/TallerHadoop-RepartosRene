...
SELECT
  ...
  (SELECT mi + (random()*(mx-mi))::int FROM pick)
...
SELECT 'Sucursal '||gs,
       (SELECT direccion_id FROM direccion ORDER BY random() LIMIT 1)
...
SELECT 'Rep_'||gs, 'RR_'||gs,
       ...
       (SELECT sucursal_id FROM sucursal ORDER BY random() LIMIT 1)
...
rnd AS (
  SELECT
    (SELECT rep_min + (random()*(rep_max-rep_min))::int FROM bounds) AS repartidor_id,
    (SELECT cli_min + (random()*(cli_max-cli_min))::int FROM bounds) AS cliente_id,
    (SELECT dir_min + (random()*(dir_max-dir_min))::int FROM bounds) AS d1,
    (SELECT dir_min + (random()*(dir_max-dir_min))::int FROM bounds) AS d2,
    (SELECT ruta_min + (random()*(ruta_max-ruta_min))::int FROM bounds) AS ruta_id,
    (SELECT des_min  + (random()*(des_max-des_min))::int FROM bounds) AS destinatario_id,
    ts
  FROM (
    SELECT timestamp '2024-01-01'
           + ((random() * (365*24))::int) * interval '1 hour' AS ts
