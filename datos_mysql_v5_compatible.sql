-- =========================================================
-- Versión compatible con MySQL 5.1 (VM Cloudera)
-- =========================================================
-- Arreglos:
-- 1. Cambiado 'utf8mb4' a 'utf8' (compatible con MySQL 5.1)
-- 2. Cambiado DATETIME DEFAULT CURRENT_TIMESTAMP a TIMESTAMP
-- 3. Eliminado 'WITH RECURSIVE'
-- 4. Reemplazada la generación de datos con Stored Procedures (bucles WHILE)
-- =========================================================
...
-- =========================================================
-- Reseñas e inconvenientes (AQUÍ ARREGLAMOS EL ERROR)
-- =========================================================
CREATE TABLE resena (
  ...
  comentario          TEXT,
  -- ARREGLO: Cambiado de DATETIME a TIMESTAMP
...
-- =========================================================
-- FIN DEL ESQUEMA (CREATE TABLE)
-- =========================================================


-- =========================================================
-- INICIO DE LA GENERACIÓN DE DATOS (COMPATIBLE CON MYSQL 5.1)
-- =========================================================

-- Cambiamos el delimitador para poder usar ';' dentro de los Stored Procedures
DELIMITER $$
...
-- 5) Direcciones (30k)
-- ================================
CREATE PROCEDURE sp_fill_direccion(IN total INT)
BEGIN
...
-- ================================
CREATE PROCEDURE sp_fill_sucursal(IN total INT)
BEGIN
  DECLARE i INT DEFAULT 1;
  START TRANSACTION;
  WHILE i <= total DO
    INSERT INTO sucursal (nombre, direccion_id)
    SELECT CONCAT('Sucursal ', i),
...
CREATE PROCEDURE sp_fill_repartidor(IN total INT)
BEGIN
  DECLARE i INT DEFAULT 1;
  START TRANSACTION;
  WHILE i <= total DO
    INSERT INTO repartidor (nombre, apellido, telefono, sucursal_id)
    SELECT CONCAT('Rep_', i), CONCAT('RR_', i),
...
-- ================================
CREATE PROCEDURE sp_fill_cliente(IN total INT)
BEGIN
  DECLARE i INT DEFAULT 1;
  START TRANSACTION;
  WHILE i <= total DO
    INSERT INTO cliente (nombre, apellido, correo, telefono, direccion_id)
    SELECT CONCAT('Nombre_', i), CONCAT('Apellido_', i),
...
CREATE PROCEDURE sp_fill_destinatario(IN total INT)
BEGIN
  DECLARE i INT DEFAULT 1;
  START TRANSACTION;
  WHILE i <= total DO
    INSERT INTO destinatario (nombre, apellido, correo, telefono)
    SELECT CONCAT('Dest_', i), CONCAT('ApDest_', i),
           CONCAT('dest', i, '@mail.com'),
           CONCAT('+56 9 ', LPAD(FLOOR(RAND()*9999999), 7, '0'));
    SET i = i + 1;
  END WHILE;
  COMMIT;
...
-- 8) Rutas (500)
-- ================================
CREATE PROCEDURE sp_fill_ruta(IN total INT)
BEGIN
  DECLARE i INT DEFAULT 1;
  START TRANSACTION;
  WHILE i <= total DO
...
-- 9) Países por ruta
-- ================================
CREATE PROCEDURE sp_fill_ruta_pais()
BEGIN
  -- 1ª vuelta
...
-- 10) Órdenes (50k)
-- ================================
CREATE PROCEDURE sp_fill_orden_transporte(IN total INT)
BEGIN
  DECLARE i INT DEFAULT 1;
  DECLARE v_fecha_envio DATETIME;
  START TRANSACTION;
  WHILE i <= total DO
    SET v_fecha_envio = TIMESTAMPADD(
      HOUR,
      FLOOR(RAND()* (365*24)),
    ...
    SELECT
      (SELECT repartidor_id FROM repartidor ORDER BY RAND() LIMIT 1),
      (SELECT cliente_id    FROM cliente    ORDER BY RAND() LIMIT 1),
      (SELECT direccion_id  FROM direccion  ORDER BY RAND() LIMIT 1),
      (SELECT direccion_id  FROM direccion  ORDER BY RAND() LIMIT 1),
      (SELECT sucursal_id   FROM sucursal   ORDER BY RAND() LIMIT 1),
      (SELECT ruta_id       FROM ruta       ORDER BY RAND() LIMIT 1),
      (SELECT destinatario_id FROM destinatario ORDER BY RAND() LIMIT 1),
      v_fecha_envio,
      DATE_ADD(v_fecha_envio, INTERVAL 3 DAY),
      DATE_ADD(v_fecha_envio, INTERVAL 3 DAY + FLOOR(RAND()*240) MINUTE),
      'entregada',
      CASE WHEN RAND() < 0.1 THEN 'Prioritaria' ELSE NULL END;
    SET i = i + 1;
  END WHILE;
  COMMIT;
END$$
...
-- 11) Reseñas
-- ================================
CREATE PROCEDURE sp_fill_resena()
BEGIN
...
-- 12) Inconvenientes
-- ================================
CREATE PROCEDURE sp_fill_inconveniente()
BEGIN
  INSERT INTO inconveniente (orden_transporte_id, descripcion, fecha_informe, tipo, resuelto)
  SELECT ot.orden_transporte_id,
         ELT(1 + FLOOR(RAND()*4), 'Retraso por clima','Paquete daniado','Direccion incompleta','Falla transporte'),
         DATE_ADD(ot.fecha_envio, INTERVAL FLOOR(RAND()*(48*60)) MINUTE),
         ELT(1 + FLOOR(RAND()*4), 'retraso','danio','direccion_invalida','operativo'),
         (RAND() < 0.7)
  FROM orden_transporte ot
  WHERE RAND() < 0.18;
END$$
...
-- =========================================================
-- LLAMAMOS A LOS PROCEDIMIENTOS PARA LLENAR LAS TABLAS
-- =========================================================
-- (Esto puede tardar varios minutos)

CALL sp_fill_direccion(30000);
CALL sp_fill_sucursal(60);
CALL sp_fill_repartidor(800);
CALL sp_fill_cliente(20000);
CALL sp_fill_destinatario(30000);
CALL sp_fill_ruta(500);
CALL sp_fill_ruta_pais();
CALL sp_fill_orden_transporte(50000);
CALL sp_fill_resena();
CALL sp_fill_inconveniente();

-- Borramos los procedimientos, ya no los necesitamos
DROP PROCEDURE sp_fill_direccion;
DROP PROCEDURE sp_fill_sucursal;
DROP PROCEDURE sp_fill_repartidor;
DROP PROCEDURE sp_fill_cliente;
DROP PROCEDURE sp_fill_destinatario;
DROP PROCEDURE sp_fill_ruta;
DROP PROCEDURE sp_fill_ruta_pais;
DROP PROCEDURE sp_fill_orden_transporte;
DROP PROCEDURE sp_fill_resena;
DROP PROCEDURE sp_fill_inconveniente;